...
def train(args):
    ...
    test_images_dir = os.path.join(args.input_path, 'test')
    image_files = []
    if os.path.exists(test_images_dir):
        image_files = sorted([f for f in os.listdir(test_images_dir) if f.endswith('.png')])

    # Calcular métricas con nombres de archivo
    metrics = calculate_metrics(generator, test_dataset, image_names=image_files, metrics_dir=trainer.metrics_dir)

    # Guardar resumen de métricas finales
    final_metrics_file = os.path.join(trainer.metrics_dir, 'final_metrics_summary.txt')
    with open(final_metrics_file, 'w') as f:
        f.write(f"Métricas Finales del Entrenamiento\n")
        f.write(f"{'='*60}\n\n")
    ...
    print(f"✓ Resumen de métricas finales guardado: {final_metrics_file}")

    print(f"\n{'='*60}")
    print("ENTRENAMIENTO COMPLETADO")
    print(f"Todos los resultados guardados en: {trainer.model_base_dir}")
    print(f"{'='*60}\n")
